...
#   scp OpenCL/opencl_runtime_* CUDA/cuda_*.run files-common/config.cfg files-ubuntu/setupUbuntuTemplate.sh mpi@$IP:~/setup/
#   /mnt/c/Program\ Files/Oracle/VirtualBox/VBoxManage.exe storageattach "Ubuntu_ClusterTemplate" --storagectl "IDE" --port 1 --device 0 --type dvddrive --medium additions
#   ssh -t mpi@$IP "sudo setup/setupUbuntuTemplate.sh"
...
echo "%sudo   ALL=(ALL) NOPASSWD: /sbin/poweroff, /sbin/reboot, /sbin/shutdown" | tee -a /etc/sudoers
...
apt-get -qq install gcc g++ gdb git make opencl-headers openmpi-bin libopenmpi1.10 libopenmpi-dev jq graphviz --assume-yes
...
mkdir /home/${SUDO_USER}/.openmpi
printf "#fixing 'A high-performance Open MPI point-to-point messaging module was unable to find any relevant network interfaces: Module: OpenFabrics (openib); Host: cluster121; Another transport will be used instead, although this may result in lower performance.'\nbtl=^openib\n" | tee -a /home/${SUDO_USER}/.openmpi/mca-params.conf
chown -R ${SUDO_USER}:${SUDO_USER} /home/${SUDO_USER}/.openmpi
